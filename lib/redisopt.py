...
def get_hash(host, uri, postdata):
    request = 'http://%s%s?' % (host, urlparse(uri).path)
    dic = urlparse(uri).query.split('&')
    for param in dic:
    ...
    for param in postdata.split('&'):
        if param != "" and "=" in param:
            request += param.split('=')[0]+'=&'
    url_hash = md5(request.encode()).hexdigest()
    return url_hash
...
def content_deal(headers, host, method, postdata, uri, packet):
    ...
    if 'Crosscan' not in headers.keys():
        request = {
            'headers': headers,
            'host': host,
            'method': method,
        ...
        b64req = base64.b64encode(json.dumps(request).encode())
