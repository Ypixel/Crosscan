...
class ConfHandler(BaseHandler):
    ...
    def post(self):
        ...
        for i in self.request.body.decode().split("&"):
            para = secure.clear(urllib.parse.unquote(i.split("=", 1)[0]))
            value = secure.clear(urllib.parse.unquote(i.split("=", 1)[1]))
...
class ScanConfigHandler(BaseHandler):
    ...
    def get(self):
        ...
        return self.render("scan_config.html", config=config.load(), start=start, rules=rules, scan_stat=config.load()['scan_stat'])
...
class ListHandler(BaseHandler):
    ...
    def get(self):
        ...
        for reqhash in content:
            ...
            #split the url in 80 chars
            req_content[reqhash] += "|"
            for i in range(int(len(req_content[reqhash].split("|")[1])/80)+1):
